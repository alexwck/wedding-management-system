...
2. **Given** the assignment dialog is open with a list of unassigned guests, **When** the user selects a guest and confirms, **Then** the chair displays the guest's name and a filled/highlighted visual indicator, and the guest is removed from the unassigned list
...
2. **Given** the reassignment dialog for an occupied chair, **When** the user clicks "Unassign", **Then** the chair returns to its empty visual state, and the guest reappears in the unassigned guests list
...
2. **Given** the unassigned guests panel is visible, **When** the user assigns a guest from the panel to a chair, **Then** the guest is removed from the panel without requiring a full page reload
...
1. **Given** a guest is assigned to "Round Table 1, Seat 3", **When** the user views the RSVP list on the dashboard, **Then** that guest's row displays "Round Table 1 — Seat 3" (or equivalent readable label)
2. **Given** a guest has no seat assignment, **When** the user views the RSVP list, **Then** that guest's row shows "Unassigned" or an empty indicator

---

### User Story 5 - Export Seat Assignments to Google Sheets (Priority: P3)

As a couple, I want exported Google Sheets data to include seat assignment columns, so that I can share a complete seating chart with my venue or coordinator.

**Why this priority**: Extends an existing export flow with additional columns. Important for day-of logistics but not blocking for the core seating workflow.

**Independent Test**: Can be tested by exporting RSVPs to Google Sheets and verifying the spreadsheet includes "Table" and "Seat" columns with correct data.

**Acceptance Scenarios**:

1. **Given** guests with mixed assignment status (some assigned, some not), **When** the user exports RSVPs to Google Sheets, **Then** the spreadsheet includes "Table Name" and "Seat" columns populated with assignment data or "Unassigned" for unseated guests
...
- What happens when a chair is moved between tables (parent changed)? The assignment persists and follows the chair to its new parent.
...
- **FR-004**: System MUST visually distinguish occupied chairs (filled/highlighted color + guest name label) from empty chairs
...
- **FR-008**: System MUST display an unassigned guests panel alongside the floor plan showing attending guests without a seat
- **FR-009**: System MUST update the unassigned guests panel in real-time as assignments are made or removed
...
- **SC-001**: Users can assign a guest to a chair in under 10 seconds (click chair → select guest → confirm)
- **SC-002**: The unassigned guests panel updates within 1 second of any assignment change
- **SC-003**: All attending guests can be assigned and unassigned without page reloads or data loss
...
- The chair's parent table determines the "table name" displayed in the RSVP list (derived from the table item's label or ID)
- Seat position within a table is derived from the chair's position/index among sibling chairs under the same parent table
- Seat assignments are per-wedding — a guest from one wedding cannot be assigned to another wedding's floor plan
- The existing `rsvps` table's `status` field ("attending"/"declined") is the source of truth for guest availability
- Google Sheets export already exists as a feature and will be extended with additional columns
...
- The assignment dialog will use a searchable list to handle weddings with many guests (50+)
