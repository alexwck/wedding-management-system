...
**Performance Goals**: `npx playwright test --project=chromium` should drop to **<= 4 minutes wall time AND <= 40% of the captured baseline** on the developer's machine. Baseline is captured in T000 before Phase 1 starts and recorded here. As of 2026-06-06, the captured baseline is **7m24s (444s)** for 160 tests in the chromium project (157 pass, 3 fail). Of the 3 failures: 2 are caused by the new default-admin storageState model (couple-dashboard.spec.ts:59 and floor-plan.spec.ts:95 expect an unauthenticated bounce to /auth/login but admin is now logged in by default - fixed by T012/T037-T040 in Phase 3), and 1 (design/admin-dashboard-audit.test.ts) is a pre-existing snapshot drift unrelated to feature 014. The win has three components in priority order:
