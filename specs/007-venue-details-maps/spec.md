...
1. **Given** a wedding has venue details saved, **When** a guest visits the landing page, **Then** a glass-panel section in the bottom gradient area (above the RSVP button) displays the couple name, wedding date, venue name, and welcome message.
...
4. **Given** a guest views the landing page on mobile, **When** the venue info section is displayed, **Then** text wraps naturally within the glass panel and the "RSVP Now" button remains fully visible below without scrolling.
...
5. **Given** a wedding has a venue name and address but no coordinates, **When** a guest visits the RSVP form page, **Then** the venue name and address are shown as text, but the map and navigation buttons are hidden.
...
- What happens when a wedding has a venue name but no address or coordinates? Display the venue name text only; hide the address, map, and navigation buttons.
- What happens when a wedding has a venue name and address but no coordinates (e.g., manually entered address)? Display the venue name and address as text; hide the map and navigation buttons.
...
- What happens when the geocoding request times out (5 seconds)? Show a "Search timed out" error; allow manual address entry without coordinates.
- What happens when the welcome message contains URLs or special characters? Display as plain text — no rendered links, no HTML interpretation, characters shown as-is.
- What happens when an autocomplete dropdown appears on mobile? Dropdown appears below the input field; tapping a result selects it and dismisses the keyboard.
...
- Q: Where should the venue info section appear on the landing page? → A: Bottom gradient area above RSVP button, glass-panel styled
- Q: Should address text be shown when coordinates are absent? → A: Yes — venue name + address shown as text, map and nav buttons hidden. Different from "venue name only" (no address) which shows just the name.
- Q: Welcome message content format? → A: Plain text only (no HTML, no rendered links, no emoji rendering — special characters displayed as-is)
- Q: Geocoding debounce interval and timeout? → A: 1000ms debounce, 5000ms timeout
...
- **FR-005**: System MUST debounce geocoding API calls with a 1000ms interval to respect the provider's rate limit, and MUST set a 5000ms request timeout. No automatic retries on failure.
- **FR-006**: System MUST require a minimum of 3 characters before triggering address autocomplete search.
- **FR-007**: System MUST display a glass-panel info section on the public landing page (within the bottom gradient area, above the RSVP button) showing couple name, wedding date, venue name, and welcome message when any of these fields are present. This section creates emotional pull to drive RSVP clicks (top of funnel).
- **FR-008**: System MUST display a logistics section on the RSVP form page showing venue name, venue address, embedded map, "Open in Maps" button, "Navigate with Waze" button, and welcome message when venue data exists. Navigation buttons MUST display side-by-side on desktop and stacked (full-width) on mobile. This section provides travel confidence to reduce form abandonment (bottom of funnel).
- **FR-009**: System MUST embed a map centered on the venue coordinates on the RSVP form page (using a free embed method that requires no paid API key). The map MUST be responsive width with a minimum height of 200px. The map is NOT shown on the landing page to keep the focus on the RSVP CTA.
- **FR-010**: System MUST provide an "Open in Maps" button linking to Google Maps at the venue coordinates (RSVP form page only). Minimum touch target 44x44px on mobile.
- **FR-011**: System MUST provide a "Navigate with Waze" button linking to Waze navigation at the venue coordinates (RSVP form page only). Minimum touch target 44x44px on mobile.
...
- **FR-013**: System MUST handle geocoding service failures gracefully by showing a descriptive error message ("Unable to search for addresses"), displaying a loading indicator during active requests, and allowing manual address text entry without coordinates. No automatic retries.
- **FR-014**: System MUST clear coordinates when the venue address field is cleared by the user.
- **FR-015**: System MUST store venue data in new columns on the existing weddings table: venue (text, max 200 chars), venue_address (text, max 500 chars), venue_lat (double precision), venue_lng (double precision), welcome_message (text, max 500 chars).
- **FR-016**: System MUST verify the user is an admin or the owning couple before allowing wedding detail updates (server-side authorization check).
- **FR-017**: System MUST treat venue_lat and venue_lng as a pair — both must be present or both null. Server-side validation MUST enforce range constraints (lat: -90 to 90, lng: -180 to 180).
- **FR-018**: System MUST display a loading indicator (spinner or "Searching..." text) during active geocoding requests in the autocomplete dropdown area.
- **FR-019**: System MUST render the welcome message as plain text only — no HTML interpretation, no rendered links, special characters displayed as-is.

### Key Entities

- **Wedding** (existing): Extended with venue (text, max 200), venue_address (text, max 500), venue_lat (double precision), venue_lng (double precision), and welcome_message (text, max 500). One venue per wedding. Coordinate pair integrity: lat+lng both present or both null.
...
- **SC-003**: Guests viewing the public landing page see venue info (date, venue name, welcome message) render within 2 seconds page load time.
- **SC-004**: Guests viewing the RSVP form page see the map and logistics section render within 2 seconds page load time (map iframe loads asynchronously and does not block form interaction).
...
- The welcome message is plain text only — no HTML, no rendered links, no emoji rendering. Special characters displayed as-is for security and simplicity.
- Concurrent editing of venue details by multiple users is not addressed — in practice, one person manages a wedding at a time. Last write wins.
- Venues are set after wedding creation on the detail page, not during the initial wedding creation flow.
- "Venue data exists" (FR-008/FR-012) means any venue-related field is populated (venue name, address, coordinates, or welcome message). The display adapts to show only the fields that have data.
